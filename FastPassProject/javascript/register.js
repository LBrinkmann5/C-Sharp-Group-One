...
//Show and Hide Password
    ...
    $("#customer_register").submit( evt =>
    {
        ...
        {
            userList.push({username, password, email, firstName, lastName, phone, address});
            
            for (let i = 0; i <= userList.length; i++)
                {
                    window.alert(`${userList[i].username}: Welcome ${userList[i].firstName} ${userList[i].lastName}`);
                }
        }
    });    
});